...
<div class="container">		
    ...
    <form action="?" method="post">
        ...
        <input type="text" id="NOM" name="NOM" required placeholder="Nom, Prénom" 
            value="<?= isset($_POST['NOM']) ? $_POST['NOM'] : '' ?>" /><br/>
        <?php 
            if(isset($_POST['NOM']) && stristr($_POST['NOM'], ',') === FALSE) {
                $valid = false;
        ?>
                <p class="red">ERREUR sur le format du nom complet !!</p>
        <?php
            }
        ?>
        <label for="EMAIL">Email :</label><br/>
        <input type="email" id="EMAIL" name="EMAIL" required placeholder="Email@monfournisseur.com" 
        ...
        <?php 
            if(isset($_POST['EMAIL']) && !preg_match ( " /^[^\W][a-zA-Z0-9_]+(\.[a-zA-Z0-9_]+)*\@[a-zA-Z0-9_]+(\.[a-zA-Z0-9_]+)*\.[a-zA-Z]{2,4}$/" , $_POST['EMAIL'] )) {
                $valid = false;
        ?>
                <p class="red">ERREUR sur le format de l'email !!</p>
        <?php
            }
        ?>
